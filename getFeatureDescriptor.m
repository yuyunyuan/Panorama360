...
Smoothedimage = imfilter(input_image, g, 'replicate','same');
...
for i = 1:npoints
patch = Smoothedimage(xp(i)-20:xp(i)+19, yp(i)-20:yp(i)+19);
patch = imresize(patch, .2);
descriptors(i,:) = reshape((patch - mean2(patch))/std2(patch), 1, 64); %compute z-score()
end