...
% disp(size(gray_A));
% disp(size(gray_B));
% gray_A=gray_A(end-543:end,end-959:end,:);
...
% hold on
% line([1,1205],[yB+size(image_B,1),yB+size(image_B,1)],'LineWidth',2,"Color","r");
% line([1,1205],[yB,yB],'LineWidth',2,"Color","r");
% plot(newH,1,'r*');
% hold off
output_image=(uint8(newImage));
imshow(output_image);
% output_image=lensdistort(output_image, -0.03);
end